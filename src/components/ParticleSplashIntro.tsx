...
export function ParticleSplashIntro() {
  ...
  //  } catch {}
    ...
    for (let i = 0; i < COUNT; i++) {
      const edge = i % 4;
      let sx = 0, sy = 0;
      if (edge === 0)      { sx = Math.random() * W; sy = -15; }
      else if (edge === 1) { sx = W + 15; sy = Math.random() * H; }
      else if (edge === 2) { sx = Math.random() * W; sy = H + 15; }
      else                 { sx = -15; sy = Math.random() * H; }
      const cx_ = CX(), cy_ = CY();
      const mx = (sx + cx_) / 2, my = (sy + cy_) / 2;
      const dx = cx_ - sx, dy = cy_ - sy;
    ...
    const tick = (now: number) => {
      ...
      for (const p of particles) {
        ...
        p.t += p.travelSpeed;
        if (p.t >= 1) {
          p.arrived = true; p.x = p.tx; p.y = p.ty;
          arrivedCount++;
          continue;
        }
        ...
        const nAngle = noise2(
          p.x * 0.004 + p.noiseOx + noiseT * 0.4,
          p.y * 0.004 + p.noiseOy,
        ) * Math.PI * 1.4;
...
function drawParticle(
  ...
  for (let i = 1; i < tLen; i++) {
    const tp = p.trail[i];
    const ratio = i / tLen;
    const a  = ratio * ratio * 0.20 * p.alpha * alphaScale;
  ...
  g.addColorStop(0,   `rgba(${PART_HEAD}, ${0.90 * p.alpha * alphaScale})`);
  g.addColorStop(0.3, `rgba(${PART_HEAD}, ${0.50 * p.alpha * alphaScale})`);
